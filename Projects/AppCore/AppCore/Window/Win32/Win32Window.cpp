...
LRESULT windowProcDispatcher(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam);
...
void Win32Window::execGrabCursor(bool isGrabbed)
{
  if (isGrabbed)
  {
    RECT rect;
    GetClientRect(m_window, &rect);
    MapWindowPoints(m_window, nullptr, reinterpret_cast<LPPOINT>(&rect), 2);
    ClipCursor(&rect);
  }
  else
  {
...
LRESULT Win32Window::processEvent([[maybe_unused]] HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  ...
  switch (msg)
  {
    ...
    case WM_CLOSE:
    {
      ...
      auto event = Event{Event::CloseEventData()};
      pushEvent(event);
      result = EXIT_SUCCESS;
      break;
    }
    case WM_SIZE:
    {
      if (wParam != SIZE_MINIMIZED && !m_resizing && m_previousSize != getSize())
      {
        ...
        auto eventData = Event::ResizeEventData();
    ...
    case WM_SETFOCUS:
    {
      ...
      auto eventData = Event::FocusEventData();
      ...
      if (m_keyRepeatEnabled || (lParam & (1 << 30)) == 0)
      {
        ...
        else
        {
          ...
          if ((character >= 0xDC00) && (character <= 0xDFFF))
          {
            ...
            CORE_ASSERT(false, "Investiguate if this can be done in std c++.");
    ...
    case WM_SYSKEYDOWN:
    {
      ...
      {
        auto eventData = Event::KeyboardEventData();
    ...
    case WM_MOUSEWHEEL:
    {
      ...
      auto eventData = Event::MouseWheelEventData();
    ...
    case WM_LBUTTONDOWN:
    {
      ...
      auto eventData = Event::MouseButtonEventData();
      eventData.button = Input::Mouse::Button::Left;
      eventData.buttonState = Event::ButtonState::Pressed;
      eventData.modifier = getKeyModifierState();
      eventData.x = position.x;
      eventData.y = position.y;
    ...
    case WM_LBUTTONUP:
    {
      ...
      auto eventData = Event::MouseButtonEventData();
      eventData.button = Input::Mouse::Button::Left;
      eventData.buttonState = Event::ButtonState::Released;
      eventData.modifier = getKeyModifierState();
      eventData.x = position.x;
      eventData.y = position.y;
    ...
    case WM_RBUTTONDOWN:
    {
      ...
      auto eventData = Event::MouseButtonEventData();
      eventData.button = Input::Mouse::Button::Right;
      eventData.buttonState = Event::ButtonState::Pressed;
      eventData.modifier = getKeyModifierState();
      eventData.x = position.x;
      eventData.y = position.y;
    ...
    case WM_MBUTTONDOWN:
    {
      ...
      auto eventData = Event::MouseButtonEventData();
      eventData.button = Input::Mouse::Button::Middle;
      eventData.buttonState = Event::ButtonState::Pressed;
      eventData.modifier = getKeyModifierState();
      eventData.x = position.x;
      eventData.y = position.y;
    ...
    case WM_XBUTTONDOWN:
    {
      ...
      auto eventData = Event::MouseButtonEventData();
      eventData.button = HIWORD(wParam) == XBUTTON1 ? Input::Mouse::Button::XButton1 : Input::Mouse::Button::XButton2;
      eventData.buttonState = Event::ButtonState::Pressed;
      eventData.modifier = getKeyModifierState();
      eventData.x = position.x;
      eventData.y = position.y;
    ...
    case WM_MOUSELEAVE:
    {
      ...
      if (m_mouseInside)
      {
        ...
        pushEvent(Event{Event::MouseLeaveEventData{}});
      }
      break;
    }
    case WM_MOUSEMOVE:
    {
      ...
      else
      {
        ...
        if (!m_mouseInside)
        {
          ...
          pushEvent(Event{Event::MouseEnteredEventData{}});
        }
      }

      auto eventData = Event::MouseMovedEventData();
      eventData.x = x;
      eventData.y = y;
    ...
    default:
    {
      result = DefWindowProc(m_window, msg, wParam, lParam);
      break;
    }
  }
